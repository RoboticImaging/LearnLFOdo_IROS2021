...
from loss_functions import multiwarp_photometric_loss, explainability_loss, smooth_loss, compute_errors, pose_loss, total_variation_loss, forward_backward_loss
...
def main():
    ...
    # for multi-warp photometric loss, we request as many depth values as the cameras used
    ...
    tb_writer.add_scalar(tag="photometric_loss_weight", scalar_value=args.photo_loss_weight)
    tb_writer.add_scalar(tag="mask_loss_weight", scalar_value=args.mask_loss_weight)    # this is not used
    tb_writer.add_scalar(tag="smooth_loss_weight", scalar_value=args.smooth_loss_weight)
...
def train(args, train_loader, disp_net, pose_net, optimizer, epoch_size, logger, tb_writer):
    ...
    for i, trainingdata in enumerate(train_loader):
        ...
        # we interpret the output of the dispnet as inverse depth
        disparities = disp_net(tgt_lf_encoded_d)    # during training disparities from two levels are predicted
        depth = [1.0 / disp for disp in disparities]    # List of 2 tensors of shape [batch_size x num_cams x hxw]
        # we interpret this to be in [x, y, z, axang] format - [batch_size x refs x 6]
        poses_est_refs_tgt = pose_net(tgt_lf_encoded_p, ref_lfs_encoded_p)

        # compute photometric error
        intrinsics = trainingdata['intrinsics'].to(device)
        poses_gt_refs_tgt = trainingdata['poses_gt_refs_tgt'].to(device)     # [batch_size x refs x 4x4 matrix]
        metadata = trainingdata['metadata']
        photometric_error, warped, diff = multiwarp_photometric_loss(
            tgt_lf, ref_lfs, intrinsics, depth, poses_est_refs_tgt, metadata, args.rotation_mode, args.padding_mode,
            sum_or_mean="mean"
        )
        ...
        mean_distance_error, mean_angle_error = pose_loss(poses_est_refs_tgt, poses_gt_refs_tgt)

        if args.forward_backward:
            poses_est_tgt_refs = pose_net(ref_lfs_encoded_p, tgt_lf_encoded_p, rev=True)      # [batch_size x refs x 6]
            fb_error = forward_backward_loss(poses_est_refs_tgt, poses_est_tgt_refs)
            loss = (args.photo_loss_weight * photometric_error +
                    args.smooth_loss_weight * smoothness_error + args.fb_loss_weight * fb_error)
        else:
            # loss = w1*photometric_error + w3*smoothness_error + w4*pose_error
            loss = (args.photo_loss_weight * photometric_error + args.smooth_loss_weight * smoothness_error)

        if log_losses:
            tb_writer.add_scalar(tag='train/photometric_error', scalar_value=photometric_error.item(),
                                 global_step=n_iter)
            tb_writer.add_scalar(tag='train/smoothness_loss', scalar_value=smoothness_error.item(),
                                 global_step=n_iter)
            tb_writer.add_scalar(tag='train/total_loss', scalar_value=loss.item(), global_step=n_iter)
            tb_writer.add_scalar(tag='train/mean_distance_error', scalar_value=mean_distance_error.item(),
                                 global_step=n_iter)
            tb_writer.add_scalar(tag='train/mean_angle_error', scalar_value=mean_angle_error.item(),
                                 global_step=n_iter)
            if args.forward_backward:
                tb_writer.add_scalar(tag='train/fb_error', scalar_value=fb_error.item(), global_step=n_iter)
...
def validate_without_gt(args, val_loader, disp_net, pose_net,
                        epoch, logger, tb_writer, sample_nb_to_log=2):
    global device
    batch_time = AverageMeter()
    if args.forward_backward:
    ...
    for i, validdata in enumerate(val_loader):
        ...
        # compute output
        # we interpret the output of the disparity network as inverse depth
        ...
        poses_gt_refs_tgt = validdata['poses_gt_refs_tgt'].to(device)        # [refs x 4x4 matrix]
        ...
        mean_distance_e, mean_angle_e = pose_loss(poses_est_refs_tgt, poses_gt_refs_tgt)   # Pose loss
        mean_distance_error = mean_distance_e.item()
        mean_angle_error = mean_angle_e.item()
        ...
        if args.forward_backward:
            poses_est_tgt_refs = pose_net(ref_lfs_encoded_p, tgt_lf_encoded_p, rev=True)  # [batch_size x refs x 6]
            fb_error = forward_backward_loss(poses_est_refs_tgt, poses_est_tgt_refs).item()
            loss = (args.photo_loss_weight * photometric_error +
                    args.smooth_loss_weight * smoothness_error + args.fb_loss_weight * fb_error)
            losses.update([loss, photometric_error, mean_distance_error, mean_angle_error, fb_error])
        else:
            loss = args.photo_loss_weight * photometric_error + args.smooth_loss_weight * smoothness_error
    ...
    if args.forward_backward:
        return losses.avg, ['val/total_loss', 'val/photometric_error', 'val/mean_distance_error', 'val/mean_angle_error', 'val/fb_error']
    else:
        return losses.avg, ['val/total_loss', 'val/photometric_error', 'val/mean_distance_error', 'val/mean_angle_error']
